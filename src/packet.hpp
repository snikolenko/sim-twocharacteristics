...
template <typename T> bool sortVoverWthenValue(const Packet<T> & i, const Packet<T> & j) { 
	if ( (i.l / (float)i.r) > (j.l / (float)j.r)) {
		return true;
	} else if ( (i.l * j.r == j.l * i.r) && (i.l > j.l) ) {
		return true;
	}
	return false;
}

template <typename T> bool sortVoverWthenWork(const Packet<T> & i, const Packet<T> & j) { 
	if ( (i.l / (float)i.r) > (j.l / (float)j.r)) {
		return true;
	} else if ( (i.l * j.r == j.l * i.r) && (i.r < j.r) ) {
		return true;
	}
	return false;
}
