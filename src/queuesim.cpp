...
void network_runsim(int k, int b_min, int b_max, int b_step, int c_min, int c_max, double beta, int num_runs) {
	...
	D("\tmaxval = " << default_maxvalue);

	// creating a generator
	PacketGenerator<int> * pg;
	if (twovalued_biased) {
		pg = (PacketGenerator<int>*)(new MMPPVectorPoissonPacketGenerator<int, MMPPoissonTwoValuedBiasedPacketGenerator<int> >(k, default_maxvalue, off_on_prob, on_off_prob, lmb, large_lmb, num_streams));
	} else if (twovalued_uniform) {
		pg = (PacketGenerator<int>*)(new MMPPVectorPoissonPacketGenerator<int, MMPPoissonTwoValuedUniformPacketGenerator<int> >(k, default_maxvalue, off_on_prob, on_off_prob, lmb, large_lmb, num_streams));
	} else if (gen_biased) {
		pg = (PacketGenerator<int>*)(new MMPPVectorPoissonPacketGenerator<int, MMPPoissonBiasedPacketGenerator<int> >(k, default_maxvalue, off_on_prob, on_off_prob, lmb, large_lmb, num_streams));
	} else {
		pg = (PacketGenerator<int>*)(new MMPPVectorPoissonPacketGenerator<int>(k, max_length, off_on_prob, on_off_prob, lmb, large_lmb, num_streams));
	}

	D("\tgenerator created");

	// populating vector of algorithms
	populate_policy_vector(v, k, b_min, b_max, b_step, c_min, c_max, max_length, max_length);
...
int main(int argc, char* argv[]) {
	...
	desc.add_options()
	    ...
	    ("min_maxval", po::value<int>(&min_maxval)->default_value(3), "min maximal value")
	    ("max_maxval", po::value<int>(&max_maxval)->default_value(100), "max maximal value")
